...
**Synced from:** `docs/` + root docs on 2026-09-17 (v**1.0.10**: session persist + auth cool-down).
...
- **Version:** 1.0.10 (`app/__version__`; see `CHANGELOG.md`)
...
**Status:** **v1.0.10** on `dev` / GHCR `:dev` (session persist + auth cool-down). **v1.0.9** remains on `main` / `:latest` until merge.
...
app/fubo/                   # client facade + DRM / lineup / schedule / session modules
...
10. Persist Fubo bearer in `config/session.json`; cool down after failed sign-in (`AUTH_COOLDOWN_SECONDS`) to avoid password-hammer lockouts
