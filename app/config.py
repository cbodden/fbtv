...
def load_settings() -> Settings:
    ...
    if not user or not password:
        raise RuntimeError(
            "Set FUBO_USER and FUBO_PASS, or create config/credentials.env "
            "(or credentials.json) on the config volume"
        )
    ...
    logger.info(
        "Fubo credentials source=%s user=%s pass_len=%d has_dollar=%s wrapping_quotes_stripped=%s",
        source,
        user,
        len(password),
        "$" in password,
