...
| [ARCHITECTURE.md](ARCHITECTURE.md) | System design, data flow, caching, metrics, DRM scan/overrides, optional `STREAM_PROXY` remux, `tvg-chno` |
...
| [API.md](API.md) | HTTP endpoints and example payloads (incl. DRM scan admin, `/ready`, M3U `tvg-chno`) |
...
App version **1.0.7** on `main` / `:latest` (see `CHANGELOG.md`). Pre-release / Unreleased on `:dev` (hygiene C + `tvg-chno` + status channel warm).
